...
async function main() {
  ...
  const userLibs = (await mxModel.getFiles()).filter((file) => file.endsWith(".jar")).filter((file) => file.includes("userlib"));
  ...
  for (const libraryName of libraryNames) {
    let count = 0;
    for (const fileName of userLibs) {
      if (fileName.includes(libraryName)) {
        count++;
      }
    }
    libraryCountMap.set(libraryName, count);
  }
